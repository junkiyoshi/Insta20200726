...
void ofApp::update() {
	...
	for (int x = -600; x <= 600; x += 600) {
		...
		for (int y = -600; y <= 600; y += 600) {
			...
			for (auto radius = 120; radius < 240; radius += 20) {

				this->setRingToMesh(this->face, this->frame, glm::vec3(x, y, 0), radius, 20, 60, 0, 180, noise_seed);
			}
		}
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end, glm::vec2 noise_seed) {
	...
	auto noise_value = radius != 0 ? glm::vec2(ofNoise(noise_seed.x - radius * 0.003 + ofGetFrameNum() * 0.0085), ofNoise(noise_seed.y - radius * 0.003 + ofGetFrameNum() * 0.0085)) :
		glm::vec2(ofNoise(noise_seed.x - 100 * 0.003 + ofGetFrameNum() * 0.0085), ofNoise(noise_seed.y - 100 * 0.003 + ofGetFrameNum() * 0.0085));
